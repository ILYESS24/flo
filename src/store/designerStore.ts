...
interface DesignerState {
  ...
  addOpenRouterModels: (models: LLMConfig[]) => void;
}
...
export const useDesignerStore = create<DesignerState>((set) => ({
  ...
  loadOpenRouterModels: async () => {
    try {
      ...
      const llmConfigs = service.convertToLLMConfigs(models);
      
      set((state) => {
        // Merge OpenRouter models with existing models, avoiding duplicates
        const existingModelIds = new Set(
          state.config.availableLLMs
            .filter(llm => llm.provider === 'openrouter')
            .map(llm => llm.name)
        );
        
        const newModels = llmConfigs.filter(config => !existingModelIds.has(config.name));
        
        return {
          config: {
            ...state.config,
            availableLLMs: [...state.config.availableLLMs, ...newModels],
          },
        };
      });
    } catch (error) {
      console.error('Failed to load OpenRouter models:', error);
      // Don't throw, just log - app should work without OpenRouter models
    }
  },

  addOpenRouterModels: (models) => {
    set((state) => {
      ...
      const newModels = models.filter(config => !existingModelIds.has(config.name));
